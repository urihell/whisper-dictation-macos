...
enum SpeechCleaner {
    ...
    /// Note: on-device generation has a large fixed per-call latency (~7s on
    /// current hardware), independent of input size — there is no warmup/caching
    /// lever to speed it up. Run only when the user opts in.
