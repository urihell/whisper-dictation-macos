...
final class IncrementalCleaner {
    ...
    // Sentence-boundary + normalization logic lives in `SentenceChunking`
    // (pure, unit-tested); thin wrappers keep call sites readable.
    private static func holdBackCut(in text: String) -> String.Index? {
        SentenceChunking.holdBackCut(in: text)
    }

    private static func normalize(_ text: String) -> String {
        SentenceChunking.normalize(text)
    }
}
